# -*- coding: utf-8 -*-
"""
Created on Sun Nov 25 17:54:37 2022
@author: Mushtaq Ali
"""
...
def smiles_to_mp(SMILES):
   ...
   if mp == None:
     ...
     mps=mp.find_next_sibling('Information').find(name='String').string
     return mps      
...
if st.sidebar.button('Predict'):
    ...
    generated_descriptors1= generate123(smiles)
...
#MolWt = Chem.Descriptors.MolWt(mol_list)
    ...
    P_mp=smiles_to_mp(smiles)
    MolWt1 = generated_descriptors1["MolWt"]
#print(MolWt1)
    Gram_liter1=(10**sol)*MolWt1 
    Gram_liter2=round(Gram_liter1, 2)
...
#Convert training fingerprints into binary, and put all training binaries into arrays
    ...
    pred_rf.rename(columns = {0:'pred_sol',1:'pred_mp'}, inplace = True)
    pred_sol=pred_rf['pred_sol']
    pred_mp=pred_rf['pred_mp']
...
# results=np.column_stack([pred_mlp,pred_xgb,pred_rf,pred_consensus])
    ...
    df_results["Predicted - LogS"]=pred_sol
    df_results["Predicted - MP"]=pred_mp
    ...
    df_results["Experiment Melting_point-PubChem"]=P_mp
...
st.write("""
# About Chem-Phy Prediction
Chem-Phy Prediction  is an  hybrid model  that has been  developed with  a decision tree algorithms and has ability to predict the Water Solubility of the compound along with Melting Point. 
Chem-Phy is developed using a quality-oriented data selection method described in [1] and trained on AqSolDB [2] largest publicly available aqueous solubility dataset.
Melting point data has been sourced from largest Chemical database Pubchem. 
AqSolPred showed a top-performance (R2 - 0.90 with Solubility and R2 -0.88 with Melting Point )
